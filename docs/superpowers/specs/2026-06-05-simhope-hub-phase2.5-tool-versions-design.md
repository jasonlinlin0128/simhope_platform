...
- **誰編**：沿用「作者可編自己工具 + admin 可編全部」，在詳情頁編輯模式新增結構化「版本」編輯區。
...
確認決策：詳情頁「目前版本 / 檔」一律以 `versions.at(-1)` 為準。

- **快速安裝 tab + sidebar 下載鈕**：下載連結與版號改讀 `versions.at(-1)`；
...
- `typeData.version` 降為 **legacy 唯讀 fallback**，不再是寫入目標。不在本輪刪除（留待日後 cleanup）。

## 4. 編輯：詳情頁編輯模式 + `VersionEditor`

在 `/tool/[id]` 既有編輯模式（`isEditMode`）內，於 block 編輯區**之外**新增一塊結構化「🕒 版本」編輯區。

- 新元件 `src/components/VersionEditor.jsx`（受控元件，props：`versions`, `type`, `onChange`）。
- 每一列欄位：
  - **版號**：text，必填。
...
- 儲存：沿用既有 `handleSave`，把 `versions` 與 `blog.blocks` 一起 `updateDoc`（同一次寫入）。
  - `handleSave` 既有寫入物件加上 `versions: localVersions`。
- 狀態：詳情頁加 `localVersions` state（初值來自 `tool.versions || []`），與 `localBlocks` 平行。
  「取消編輯」時一併還原 `localVersions`。
...
1. **下載顯示改讀 versions[]**：download / doc / skill 的快速安裝 tab 與 sidebar 下載鈕是迴歸重點 —
   migrate 後與「versions 為空時的 fallback」兩條路徑都要實測能下載。
2. **block 編輯管線**：版本編輯區與 block 編輯區共用同一 `handleSave` / 同一 edit topbar —
   加完要確認既有 8 種 block 編輯 / 儲存 / 取消還原沒壞。
3. **tab 插入邏輯**：版本 tab 依資料有無動態出現，`activeTab` 修正邏輯（`tabs.some(...)` fallback）
   要確認切換 / 重整 / 編輯後不會卡在不存在的 tab。
4. **migration 的 date 推導**：`createdAt` / `updatedAt` 可能是 Firestore Timestamp 或 JS Date 或缺，
   `toYMD()` 要容錯（無法判定就留空字串，前台日期可缺省）。
5. **排序約定**：務必維持「`at(-1)` = 目前版」，否則破壞既有 `:704`/`:1073` 讀取與下載鈕。

## 10. 驗收標準

- [ ] 詳情頁編輯模式可新增 / 刪除 / 上下移版本；download/doc/skill 列可上傳檔並存到 Storage；webapp/mcp/api 列無上傳欄。
- [ ] 儲存後 `versions` 正確寫入；既有 block 編輯 / 儲存 / 取消無迴歸。
- [ ] 「🕒 版本」tab 僅在有版本時出現，內容新 → 舊，markdown 說明正確渲染，有檔的版本可下載舊版。
- [ ] 快速安裝 tab + sidebar 下載鈕改讀 `versions.at(-1)`，空 versions 時 fallback 舊欄位仍可下載（無迴歸）。
- [ ] 「最後更新」顯示最新版日期。
