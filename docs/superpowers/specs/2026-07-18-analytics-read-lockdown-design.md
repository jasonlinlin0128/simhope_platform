...
`src/lib/toolIds.js` 的 `getAllToolIdSet()` 註解明確記錄了一個刻意決定（commit `2fbc6ee`）：`/api/track`／`/api/tool-helpful` 驗證 `toolId` 時故意用 Admin SDK 列出**全部**工具 id（不管 `visibility`），避免受限（`BY_RULE`/`HIDDEN`）app 的計數被靜默丟棄——這是 PR-4 Registry ACL 上線時修的一個真實 bug。

問題是：`toolViews`/`toolHelpful` 這兩份文件因此含有**所有**工具（含被 Registry ACL 隱藏的）的 id 與計數，而 rules 讓任何人（含匿名）能直接用 client SDK 整份讀出來——這正好繞過了同一個 PR 才剛收斂好的 `canSeeApp`/`canOpenApp` 可見性規則：不需要登入、不需要猜 UI，只要照 Firebase 專案設定（本來就是 public 的 client config）呼叫一次 `getDoc(doc(db,'analytics','toolViews'))`，就能拿到「有哪些工具存在＋各自熱門度」的完整清單，包含理論上看不到的那些。
...
| `HelpfulButton.jsx`                                                   | 瀏覽器，掛在 `/tool/[id]` 詳情頁，**任何訪客都會看到**（設計上就是公開數字，只有投票要登入）                  | `getDoc(doc(db,'analytics','toolHelpful'))` 後取 `s.data()[toolId]`                                                                                                                                   | **這是目前唯一真正合法、且非 admin 也需要的讀取**——但它整份讀回瀏覽器，devtools 能看到其他工具（含隱藏的）的計數，即使畫面只顯示一個數字                            |
| `serverCatalog.js` 的 `getServerToolViews()`/`getServerToolHelpful()` | Server Component（RSC），匿名 Firestore REST（`fetch` 無 auth header，走跟 client SDK 一樣的 rules）          | 整份抓回伺服器，但只把值接到已經用 `visibility=PUBLIC_ALL` 篩過的 `getServerCatalog()` 工具清單上（`attachHelpfulCounts`/`rankPopularTools` 只 `map` 給定的 tools 陣列，不會把整份 map 序列化進頁面） | **不洩漏**——原始 map 只在伺服器記憶體出現，從未進到瀏覽器                                                                                                           |
...
`getServerToolViews()`/`getServerToolHelpful()` 從匿名 `fetch(...):runQuery`/REST 改成比照 `getAllToolIdSet()` 的寫法，用 `getAdmin().adminDb` 直接讀（伺服器對伺服器，繞過 rules，本來就是可信邊界內）。回傳值形狀（`Record<string, number>`，`pickNumericFields` 過濾非數值欄位）完全不變，呼叫端（`app/page.jsx`、`app/hub/page.jsx`）不用改。

`getServerMetrics()`（讀 `totals`）不動——規則沒收緊，繼續用匿名 REST。

### 3. 新 API：`GET /api/tool-helpful/[toolId]`

比照 `app/api/apps/[id]/open/route.js` 的寫法（`enforceRateLimit` + `HttpError`/`handleApiError`）：
...
不存在的 `toolId`（打錯字、亂猜、甚至猜中一個隱藏工具的 id）一律自然回 `0`——不用額外查 `getAllToolIdSet()` 驗證存在性，因為「查無此工具」跟「存在但目前 0 個讚」本來就該回傳同一種結果（同一個道理跟 `/api/apps/[id]/open` 用 404 統一「不存在」跟「看不到」一致：不主動幫攻擊者分辨兩種情況）。
...
- 不拆成 public/all 兩份 doc（`toolViewsPublic`/`toolViewsAll`）。討論過這個方案：需要 writer 在寫入當下查 `tool.visibility` 決定寫哪份、需要 migration 備份現有資料、admin 讀取路徑要換 collection——複雜度遠高於「把兩支伺服器函式從 REST 換成 Admin SDK + 加一支 scoped API」，且沒有帶來額外的安全收益（伺服器端信任邊界內讀取本來就不會外洩）。
- 不改 `/api/track`、`/api/tool-helpful`（POST 版）的寫入邏輯——`2fbc6ee` 的修正（受限 app 計數不被丟棄）完全不受影響，這次只動讀取路徑。
