...
- `type` 的 enum 新增 `skill`；對應的 badge/CTA/typeData 編輯器分支集中到 `taxonomy.js`（見 §2.6）。
- Skill zip 上傳沿用現有 `src/lib/storage.js` + `UploadButton.jsx`（v0.6 已建），新 `pathPrefix: 'skills'`。
...
### 2.6 taxonomy 集中化（refinement — 對清晰度幫助大）

> 探勘現有碼發現：type 的「label / emoji / badge 樣式 / CTA 文案 / 專屬欄位」目前**散落在 5+ 個檔案各自重定義**，每個都要手動同步。加 `skill`（第 7 種 type）會放大這個痛點。

現況重複處（加 skill 時全部要改）：

| 檔案                                  | 重複定義的常數                                   | 用途                                           |
| ------------------------------------- | ------------------------------------------------ | ---------------------------------------------- |
| `app/page.jsx`                        | `TYPE_CHIPS`                                     | 首頁類型 chip（→ 改為 /hub 的 `CategoryTabs`） |
| `src/components/ToolCard.jsx`         | `TYPE_BADGES` + `getCTA` 的 `ctaByType`          | 卡片 badge + CTA                               |
| `app/dashboard/page.jsx`              | `TYPE_OPTIONS`                                   | 上傳表單類型選項                               |
| `src/components/ReviewToolWizard.jsx` | `TYPE_LABELS` + `TypeDataEditor`                 | 審核 wizard                                    |
| `app/tool/[id]/page.jsx`              | `TYPE_ACTION` + `DetailTabs` + `QuickInstallTab` | 詳情頁 tab/CTA                                 |

**做法**：新增 `src/lib/taxonomy.js` 作為**單一真相來源**，匯出：

- `CATEGORIES`：5 類別的 `{ key, label, emoji, desc, defaultType }`（platform/tool/project/mcp/skill）
- `TYPES`：7 種交付格式的 `{ key, label, emoji, badgeCls, cta, action }`（webapp/download/doc/mcp/api/embedded/**skill**）
- helper：`getCTA(tool)`、`getTabsForType(type)`、`typeDataFields(type)`（給 wizard 動態渲染）

上述各檔改成 **import 自 taxonomy.js**，不再各自定義。這是「改到的碼順手收斂」，直接服務你要的可讀性/清晰度，且讓「加第 8 種 type」未來只改一處。

### 2.7 上傳 UX（category 主選、type 由 admin 細修）

- **dashboard 提交表單**：把「④ 類型」主選項從 7 種 `type` 換成 **5 種 `category`**（平臺/工具/專案/MCP/Skill — 對非技術同仁更直覺）。
- 送出時依 `category` 帶 `defaultType`（mcp→mcp、skill→skill、platform/tool/project→webapp），`type` 細節留給 admin 在 wizard 細修。
- **ReviewToolWizard**：Step 2 加 `category` 下拉（可改）+ `type` 下拉（沿用）；`TypeDataEditor` 加 `skill` 分支（`skillZipUrl` 用 `UploadButton pathPrefix="skills"` + `version` + `installPath`），platform/project 分支補對應欄位。
...
5. **上傳/審核擴充**：`dashboard` 表單主選改 `category`（§2.7）+ `ReviewToolWizard` 加 `category` 下拉與 `skill` typeData 分支；skill 類別顯示 zip `UploadButton`。
6. **taxonomy 集中化**（§2.6）：新增 `src/lib/taxonomy.js`（`CATEGORIES` / `TYPES` / helper），上述 5 處散落定義改 import。
7. **`db.js`**：新增 `getCatalog({category})`（`CATEGORIES` 移到 taxonomy.js）。
8. **順手修 bug**：`app/tool/[id]/page.jsx` `DetailTab` 的 `descHtml` 未定義 → 改用 `tool.desc`，修掉「無 desc 且無 blocks」時的 crash。
9. **migration**：`migrate-category.mjs`（依 §2.5 順序）。
...
| 資料     | `src/lib/db.js`                     | 擴充            | `getCatalog({category})`                     |
| taxonomy | `src/lib/taxonomy.js`               | 新增            | `CATEGORIES` / `TYPES` / `getCTA` 等單一真相 |
| 卡片邏輯 | `ToolCard.jsx`                      | 擴充            | 改 import taxonomy；加 skill badge/CTA       |
| 上傳     | `dashboard` `TYPE_OPTIONS`          | 改              | 主選改 5 類別 category（§2.7）               |
| 審核     | `ReviewToolWizard` `TypeDataEditor` | 擴充            | 加 category 下拉 + skill typeData 分支       |
| 詳情修正 | `app/tool/[id]/page.jsx`            | 修 bug          | `descHtml` 未定義 → `tool.desc`              |
...
6. **taxonomy 重構的迴歸風險**（§2.6）：抽 `taxonomy.js` 會動到首頁卡片 / 詳情頁 / dashboard / wizard 5 個面。每改一處要實測該畫面（卡片 CTA、詳情 tab、上傳選項、審核欄位）沒壞，再進下一處。一次抽一處、逐一驗證，別五處一起改。
...
- [ ] taxonomy.js 抽出後，5 個面（卡片 CTA / 詳情 tab / dashboard 選項 / wizard 欄位 / 首頁）外觀行為不變（無迴歸）。
- [ ] 詳情頁開「無 desc 且無 blocks」的工具不再 crash（descHtml bug 已修）。
- [ ] `npm run build` 通過。
