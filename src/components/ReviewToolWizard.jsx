...
const COLOR_OPTIONS = [
    { key: 'c1', cls: 'bg-gradient-to-br from-blue-100 to-blue-200', textCls: 'text-blue-700' },
    { key: 'c2', cls: 'bg-gradient-to-br from-red-100 to-red-200', textCls: 'text-red-700' },
    { key: 'c3', cls: 'bg-gradient-to-br from-purple-100 to-purple-200', textCls: 'text-purple-700' },
    { key: 'c4', cls: 'bg-gradient-to-br from-amber-100 to-amber-200', textCls: 'text-amber-700' },
    { key: 'c5', cls: 'bg-gradient-to-br from-emerald-100 to-emerald-200', textCls: 'text-emerald-700' },
    { key: 'c6', cls: 'bg-gradient-to-br from-pink-100 to-pink-200', textCls: 'text-pink-700' },
];
            ...
            <div className="flex justify-between items-start mb-6 pb-4 border-b border-gray-100 dark:border-gray-700">
                <div>
                    <h3 className="text-2xl font-black text-[var(--color-text-dark)] flex items-center gap-2">
                        🔍 審核 wizard — <span className="text-[var(--color-clay-purple)]">{tool.title}</span>
                    </h3>
                    <p className="text-sm text-[var(--color-text-mid)] mt-1">
                        Tool ID: <code className="text-xs">{tool.id}</code> · 狀態：{tool.status} · 作者：{tool.authorUid || '(無)'}
                    </p>
                </div>
                <button onClick={onClose} className="px-4 py-2 rounded-lg bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 font-bold text-sm hover:bg-gray-200">✕ 關閉</button>
            </div>
            ...
            {step === 1 && (
                <div className="flex flex-col gap-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <FieldRow label="④ 類型" value={`${TYPE_LABELS[tool.type]?.emoji || ''} ${TYPE_LABELS[tool.type]?.label || tool.type}`} />
                    </div>
                    ...
                    <div className="text-xs text-[var(--color-text-mid)] italic mt-2">
                        提交時間：{tool.createdAt?.toDate ? tool.createdAt.toDate().toLocaleString() : '(無)'}
                    </div>
                    <div className="flex justify-end mt-4">
                        <button onClick={() => setStep(2)} className="px-6 py-3 rounded-xl bg-[var(--color-clay-purple)] text-white font-extrabold shadow hover:-translate-y-0.5 transition">
                            進入下一步 →
                        </button>
                    </div>
                </div>
            ...
            {step === 2 && (
                <div className="flex flex-col gap-5">
                    ...
                    <FormField label="desc — 詳細說明 (Pattern C：Before/After)">
                        <textarea value={form.desc} onChange={e => update({ desc: e.target.value })} rows={5}
                    ...
                    <div className="bg-[var(--color-card-bg)]/50 border border-dashed border-[var(--color-clay-purple)]/30 rounded-2xl p-4">
                        <h4 className="font-extrabold text-sm text-[var(--color-text-dark)] mb-3">
                            🔧 類型專屬欄位 (typeData) — 依目前類型 [{TYPE_LABELS[form.type]?.label}]
                        </h4>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <FormField label="專案目錄 (folder)">
                            <input value={form.folder} onChange={e => update({ folder: e.target.value })} placeholder="例：日報表專案"
                                className="w-full bg-gray-50 dark:bg-gray-700 p-2 rounded-lg border border-gray-200 dark:border-gray-600 text-sm" />
                        </FormField>
                    </div>
                    <FormField label="適用場景 (用逗號分隔)">
                        <input value={form.scenarios} onChange={e => update({ scenarios: e.target.value })} placeholder="生產現場, 工時統計"
                            className="w-full bg-gray-50 dark:bg-gray-700 p-2 rounded-lg border border-gray-200 dark:border-gray-600 text-sm" />
                    </FormField>
                    <FormField label="標籤 tags (用逗號分隔)">
                        <input value={form.tags} onChange={e => update({ tags: e.target.value })} placeholder="AI, RAG, Teams"
                            className="w-full bg-gray-50 dark:bg-gray-700 p-2 rounded-lg border border-gray-200 dark:border-gray-600 text-sm" />
                    </FormField>
                    ...
                    <FormField label="卡片底色">
                        <div className="flex gap-2 flex-wrap">
                            {COLOR_OPTIONS.map(c => (
                                <button key={c.key} type="button" onClick={() => update({ color: c.key })}
                                    className={`w-10 h-10 rounded-xl ${c.cls} ${form.color === c.key ? 'ring-2 ring-[var(--color-clay-purple)] ring-offset-2' : ''}`}
                                    aria-label={c.key}>
                                    {form.color === c.key && <span className="text-white font-black">✓</span>}
                                </button>
                            ))}
                        </div>
                    </FormField>
                    ...
                    <FormField label="blog.summary（詳情頁短摘要，選填）">
                        <textarea value={form.blogSummary} onChange={e => update({ blogSummary: e.target.value })} rows={2}
                            className="w-full bg-gray-50 dark:bg-gray-700 p-2 rounded-lg border border-gray-200 dark:border-gray-600 text-sm" />
                        <p className="text-xs text-gray-400 mt-1">註：詳細說明的 block editor（圖文/影音步驟）請到「詳情頁 → 編輯」加</p>
                    </FormField>

                    <div className="flex justify-between mt-4">
                        <button onClick={() => setStep(1)} className="px-5 py-2.5 rounded-xl bg-gray-100 dark:bg-gray-700 text-[var(--color-text-mid)] font-bold">← 上一步</button>
                        <div className="flex gap-2">
                            ...
                            <button onClick={() => setStep(3)} className="px-6 py-2.5 rounded-xl bg-[var(--color-clay-purple)] text-white font-extrabold shadow">
                                預覽 + 上架 →
                            </button>
                        </div>
                    </div>
                ...
                <div className="flex flex-col gap-5">
                    ...
                    <div className="bg-[var(--color-card-bg)] rounded-2xl border border-[var(--color-card-border)] p-5 max-w-sm shadow">
                        <div className="flex items-center gap-3 mb-2">
                            <div className={`w-12 h-12 rounded-2xl ${COLOR_OPTIONS.find(c => c.key === form.color)?.cls || 'bg-gray-100'} flex items-center justify-center text-2xl`}>
                                {form.icon}
                            </div>
                            <div className="flex-1 min-w-0">
                                <h4 className="font-extrabold text-sm truncate">{form.title}</h4>
                                <span className="text-[0.65rem] inline-block px-1.5 py-0.5 rounded font-bold bg-gray-100 dark:bg-gray-700">
                                    {TYPE_LABELS[form.type]?.emoji} {TYPE_LABELS[form.type]?.label}
                                </span>
                            </div>
                        </div>
                        <p className="text-xs text-[var(--color-text-mid)] line-clamp-2">{form.tagline}</p>
                    </div>
                    ...
                    <div className="bg-[var(--color-card-bg)]/50 border border-dashed border-gray-300 dark:border-gray-600 rounded-2xl p-4">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                            {[
                                { val: 'live',  label: '🟢 使用中', cls: 'bg-green-50 text-green-700 border-green-300' },
                                { val: 'beta',  label: '🟠 測試中', cls: 'bg-orange-50 text-orange-700 border-orange-300' },
                                { val: 'new',   label: '🌟 新上線', cls: 'bg-blue-50 text-blue-700 border-blue-300' },
                                { val: 'dev',   label: '🔨 開發中', cls: 'bg-gray-100 text-gray-700 border-gray-300' },
                            ].map(s => (
                    ...
                    <div className="flex justify-between gap-3 mt-4 flex-wrap">
                        <button onClick={() => setStep(2)} className="px-5 py-2.5 rounded-xl bg-gray-100 dark:bg-gray-700 text-[var(--color-text-mid)] font-bold">← 上一步</button>
                        <div className="flex gap-2 flex-wrap">
                            <button onClick={handleReject} disabled={saving}
                                className="px-5 py-2.5 rounded-xl bg-red-50 text-red-600 font-bold border-2 border-red-300 disabled:opacity-50">
                                🗑️ 退回（刪除）
                            </button>
                            <button onClick={() => handleSaveOnly('pending')} disabled={saving}
                                className="px-5 py-2.5 rounded-xl bg-yellow-50 text-yellow-700 font-bold border-2 border-yellow-300 disabled:opacity-50">
                                💾 先存草稿 (pending)
                            </button>
                            <button onClick={() => handleSaveOnly(form.status)} disabled={saving}
...
function FieldRow({ label, value, mono }) {
    return (
        <div>
            <label className="text-xs font-extrabold text-[var(--color-text-mid)] uppercase tracking-wider">{label}</label>
            <p className={`mt-1 text-[var(--color-text-dark)] ${mono ? 'font-mono text-sm break-all' : 'font-bold'}`}>
                {value || '(未填)'}
            </p>
        </div>
    );
...
function FormField({ label, children }) {
    return (
        <div>
            <label className="block text-xs font-extrabold text-[var(--color-text-mid)] mb-1">{label}</label>
            {children}
        </div>
    );
}
...
function TypeDataEditor({ type, td, updateTd }) {
    if (type === 'webapp') {
        return <p className="text-sm text-[var(--color-text-mid)] italic">webapp 類型不需要額外 typeData，main url 就夠。</p>;
    }
        ...
        return (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                <FormField label="version">
                    <input value={td.version || ''} onChange={e => updateTd({ version: e.target.value })} placeholder="v1.2.0"
                        className="w-full bg-gray-50 dark:bg-gray-700 p-2 rounded-lg border border-gray-200 dark:border-gray-600 text-sm" />
                </FormField>
                <FormField label="fileName">
                    <input value={td.fileName || ''} onChange={e => updateTd({ fileName: e.target.value })} placeholder="installer.exe"
                        className="w-full bg-gray-50 dark:bg-gray-700 p-2 rounded-lg border border-gray-200 dark:border-gray-600 text-sm font-mono" />
                </FormField>
            </div>
        );
    ...
    if (type === 'doc') {
        return (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                <FormField label="version">
                    <input value={td.version || ''} onChange={e => updateTd({ version: e.target.value })} placeholder="v2026.05"
                        className="w-full bg-gray-50 dark:bg-gray-700 p-2 rounded-lg border border-gray-200 dark:border-gray-600 text-sm" />
                </FormField>
                <FormField label="fileName">
                    <input value={td.fileName || ''} onChange={e => updateTd({ fileName: e.target.value })} placeholder="表單名.docx"
                        className="w-full bg-gray-50 dark:bg-gray-700 p-2 rounded-lg border border-gray-200 dark:border-gray-600 text-sm font-mono" />
                </FormField>
            </div>
        );
        ...
        return (
            <div className="flex flex-col gap-3">
                <FormField label="mcpbUrl (.mcpb 一鍵安裝包連結)">
                    <input value={td.mcpbUrl || ''} onChange={e => updateTd({ mcpbUrl: e.target.value })}
                        placeholder="https://.../iso-form.mcpb"
                        className="w-full bg-gray-50 dark:bg-gray-700 p-2 rounded-lg border border-gray-200 dark:border-gray-600 text-sm font-mono" />
                </FormField>
                <FormField label="npmPackage">
                    <input value={td.npmPackage || ''} onChange={e => updateTd({ npmPackage: e.target.value })}
                        placeholder="@simhope/iso-form-mcp 或 github:simhope/iso-form-mcp"
                        className="w-full bg-gray-50 dark:bg-gray-700 p-2 rounded-lg border border-gray-200 dark:border-gray-600 text-sm font-mono" />
                </FormField>
                <FormField label="repoUrl">
                    <input value={td.repoUrl || ''} onChange={e => updateTd({ repoUrl: e.target.value })}
                        placeholder="https://github.com/simhope/..."
                        className="w-full bg-gray-50 dark:bg-gray-700 p-2 rounded-lg border border-gray-200 dark:border-gray-600 text-sm font-mono" />
                </FormField>
                <FormField label="configSnippet（給 Cursor / VSCode 貼到 mcp config 的 JSON）">
                    <textarea value={td.configSnippet || ''} onChange={e => updateTd({ configSnippet: e.target.value })} rows={6}
        ...
        return (
            <div className="flex flex-col gap-3">
                <FormField label="endpoint">
                    <input value={td.endpoint || ''} onChange={e => updateTd({ endpoint: e.target.value })}
                        placeholder="https://api.simhope.local/..."
                        className="w-full bg-gray-50 dark:bg-gray-700 p-2 rounded-lg border border-gray-200 dark:border-gray-600 text-sm font-mono" />
                </FormField>
                <FormField label="docsUrl">
                    <input value={td.docsUrl || ''} onChange={e => updateTd({ docsUrl: e.target.value })}
