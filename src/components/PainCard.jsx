...
export default function PainCard({ card }) {
  const { before, after, scenarios, relatedToolId } = card;
    ...
    <>
      ...
      <div className="flex gap-2 flex-wrap mb-4 relative z-10">
        {scs.map((s, idx) => (
          <span
            key={idx}
            className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm text-[0.75rem] px-2.5 py-1 rounded-full font-bold text-[var(--color-clay-purple)] shadow-sm border border-gray-100/50"
          >
            {s}
          </span>
        ))}
