...
const REVISIONS = {
  ...
  pc_ai_1772085135196_3: {
    action: "delete",
    reason: "電控部日報 dup of 5q7qjkx",
  },
...
async function main() {
  ...
  if (notFound.length > 0) {
    ...
    for (const card of notFound) {
      console.log(
        `  ${card.id.padEnd(28)} ${(card.before || "").slice(0, 50)}`,
      );
    }
  }
  ...
  for (const { card, rev } of toUpdate) {
    ...
    console.log(
      `  ✅ ${card.id.padEnd(28)} 備份 + relatedToolId=${rev.relatedToolId || "null"}`,
    );
  }
