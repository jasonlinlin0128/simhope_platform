...
const CREATIONS = [
    {
        ...
        data: {
            title: 'SimHope 工具箱',
            tagline: 'Windows 桌面整合工具集 — PDF / Office / 製造 ERP / 通用 共 32 個模組',
...
function computeNewFields(tool) {
    ...
    if (special.status && special.status !== tool.status) changes.status = special.status;
...
function formatDiff(tool, changes) {
    ...
    for (const [key, newValue] of Object.entries(changes)) {
        const oldValue = tool[key] === undefined ? '(無)' : JSON.stringify(tool[key]);
        const newValueStr = JSON.stringify(newValue);
        const truncate = (s, n) => s.length > n ? s.slice(0, n) + '…' : s;
        lines.push(`      ${key}: ${truncate(oldValue, 60)}  →  ${truncate(newValueStr, 60)}`);
    }
...
async function main() {
    ...
    console.log(`找到 ${allDocs.length} 筆，其中 ${toUpdate.length} 筆 update / ${toDelete.length} 筆 delete / 將新增 ${CREATIONS.length} 筆\n`);
    ...
    for (const c of toCreate) {
        ...
        console.log(`      type=${c.data.type}  status=${c.data.status}  folder="${c.data.folder}"`);
    ...
    for (const { tool, changes, has } of updatePlans) {
        ...
        console.log(`  ✅ [UPDATE] ${tool.id}  備份 + ${Object.keys(changes).length} 個欄位變更`);
    }
