...
const CHANGES = {
  t8: {
    ...
    typeData: {
      ...
      accessNote:
        "工具部署在機敏辦公室的影印機環境，列印時自動加時間戳浮水印。需要調整或查詢請找 MIS / 資安小組。",
      contact: "MIS / 資安小組",
    },
  },
  t9: {
    ...
    typeData: {
      ...
      accessNote:
        "DLL 外掛安裝在加工部的 MasterCAM 2025 環境，於該台電腦操作即可一鍵匯出客製化報表。目前仍在開發/評估中。",
      contact: "經企室",
    },
  },
};
...
async function main() {
  ...
  for (const { tool, change } of plans) {
    ...
    console.log(
      `             →  ${JSON.stringify({ ...(tool.typeData || {}), ...change.typeData })}`,
    );
  }
