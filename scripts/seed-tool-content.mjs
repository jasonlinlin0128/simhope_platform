...
const TOOL_DRAFTS = [
  // ── 現場即時翻譯 ──────────────────────────────────────────────────────────
  {
    title: "現場即時翻譯",
    summary:
      "這是我在工廠現場為泰籍員工打造的即時三語翻譯工具，解決跨語言溝通效率問題。",
    blocks: [
      {
        id: uid(),
        type: "text",
      ...
      },
      {
        ...
        type: "tip",
        content:
          "這個工具設計給非技術背景的現場師傅使用，直接開啟連結即可，不需安裝任何 App。",
      },
    ],
  },
  ...
  // ── 電控部日報表 ──────────────────────────────────────────────────────────
  {
    title: "電控部日報表",
    summary:
      "電控部專案工時線上管理，支援有無訂單單號的任務，主管即時掌握進度。",
    blocks: [
      {
        id: uid(),
        type: "text",
  ...
  // ── 內部文件問答庫 ────────────────────────────────────────────────────────
  {
    title: "內部文件問答庫",
    summary:
      "把公司 SOP 和技術文件變成可以用中文直接問答的知識庫，讓老師傅的經驗傳承下來。",
    blocks: [
      {
        id: uid(),
        type: "text",
      ...
      },
      {
        ...
        type: "tip",
        content:
          "文件越完整、答案越精準。建議優先上傳最常被問到的 SOP 和技術規範。",
      },
    ],
  },
  ...
  // ── 批量電子簽章工具 ──────────────────────────────────────────────────────
  {
    title: "批量電子簽章工具",
    summary:
      "取代列印→蓋章→掃描的流程，在 PDF 上直接加文字與簽名，一次處理多頁。",
    blocks: [
      {
        id: uid(),
        type: "text",
      ...
      },
      {
        ...
        type: "tip",
        content:
          "第一次使用時需要先建立你的簽名圖檔，建議用白紙黑字手寫後拍照裁切備用。",
      },
    ],
  },
  ...
  // ── 修改檔案日期工具 ──────────────────────────────────────────────────────
  {
    title: "修改檔案日期工具",
    summary:
      "快速調整電腦檔案的建立或修改時間，適用對齊文件日期或整理歷史檔案的場景。",
    blocks: [
      {
        id: uid(),
        type: "text",
      ...
      },
      {
        ...
        type: "warning",
        content:
          "此工具僅供檔案整理用途，請勿用於偽造或竄改任何正式文件的時間記錄。",
      },
    ],
  },
  ...
  // ── Teams Bot（RAG 聊天機器人）────────────────────────────────────────────
  {
    ...
    summary:
      "這是我在 Teams 上整合 RAG 聊天機器人的完整歷程，從技術選型到最終上線。",
    blocks: [
      {
        id: uid(),
        type: "text",
      ...
      },
      {
        ...
        type: "tip",
        content:
          "目前知識庫涵蓋範圍以電控部技術文件為主，如需擴充涵蓋範圍請聯絡 Jason。",
      },
    ],
  },
  ...
  // ── 遠地出差工具 ──────────────────────────────────────────────────────────
  {
    title: "遠地出差工具",
    summary:
      "出差申請從紙本搬到線上，填表、審核、確認一條龍，不用再跑三個單位蓋章。",
    blocks: [
      {
        id: uid(),
        type: "text",
  ...
  // ── 機敏辦公室影印機浮水印 ────────────────────────────────────────────────
  {
    ...
    blocks: [
      ...
      },
      {
        ...
        type: "tip",
        content:
          "浮水印採淺色背景印刷，不影響文件正常閱讀，但在白紙上清晰可見。",
      },
    ],
  },
  ...
  // ── MasterCAM2025 外掛報表 ────────────────────────────────────────────────
  {
    ...
    summary:
      "MasterCAM 2025 外掛開發記錄，讓加工部能在熟悉環境中匯出符合格式需求的報表。",
    blocks: [
      {
        id: uid(),
        type: "text",
  ...
  // ── SimHope 行事曆 ────────────────────────────────────────────────────────
  {
    title: "SimHope 行事曆",
    summary:
      "為 SimHope 量身打造的行事曆，補足 TimeTree 與 Notion 在公司多部門使用情境的不足。",
    blocks: [
      {
        id: uid(),
        type: "text",
  ...
  // ── 物管籌料數位化作業系統 ───────────────────────────────────────────────
  {
    title: "物管籌料數位化作業系統",
    summary:
      "數位化看板即時顯示所有工單的籌料進度，解決紙本非同步傳遞的問題。（已終止）",
    blocks: [
      {
        id: uid(),
        type: "text",
...
async function seed() {
  ...
  console.log(
    ">>> 記得之後到各工具詳情頁補上截圖 URL（image block 的 content 欄位）",
  );
}
  ...
  for (const t of tools) {
    ...
    const files =
      t.files && t.files.length > 0
        ? t.files
            .map((f) => f.name || f.url || "(unnamed)")
            .join(", ")
            .slice(0, 30)
        : "（無）";
    console.log(
      `${t.id.padEnd(8)} ${(t.title || "").padEnd(30)} ${url.padEnd(47)} ${files}`,
    );
  }
}
...
const runner = args.includes("--list")
  ? listTitles
  : args.includes("--check-urls")
    ? checkUrls
    : args.includes("--patch-urls")
      ? patchUrls
      : seed;
runner().catch((err) => {
